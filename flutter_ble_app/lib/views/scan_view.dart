...
Future<void> _getCharacteristic({required int selectedDeviceIndex}) async {
  try {
    ...
    developer.log("Discovered Services:");
    for (BluetoothService service in services) {
      developer.log("Service UUID: ${service.uuid}");
      for (BluetoothCharacteristic characteristic in service.characteristics) {
        developer.log("  Characteristic UUID: ${characteristic.uuid}");
        if (characteristic.uuid.toString() == voltageCharacteristicUuid) {
    ...
    if (_selectedCharacteristic == null) {
      Fluttertoast.showToast(
        msg: 'Characteristic not found. Please check the device.',
      );
    ...
    Fluttertoast.showToast(
      msg: '${_scanResults[selectedDeviceIndex].device.name} connected',
    );
  } catch (e) {
    developer.log('Error discovering characteristic: $e', level: 1000);
    setState(() => _isConnecting = false);
    rethrow;
  }
}
